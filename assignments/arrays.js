...
console.log(`\n\n\n\nARRAYS.JS PAGE vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv\n`)
...
var item = inventory.find(item => item.id === 33);
...
console.log(`\n LIST OF CARS ALPHABETICALLY BY MODEL NAME\n`)
for (let m = 0; m < carModels.length; m++) {
    console.log(carModels[m].car_model);
}
console.log(`\n`)
...
var older_cars = []
car_years.forEach(function(number){
    if(number > 2000)
    {
        older_cars.push(number);
    }
});
console.log(`\n There are`, older_cars.length, `cars built after year 2000\n`);
...
for (let t = 0; t < inventory.length; t++) {
    if (inventory[t].car_make === "BMW" || inventory[t].car_make === "Audi") {
        BMWAndAudi.push(inventory[t]);
    }
}
console.log(`\n There are`, BMWAndAudi.length, `BMW or Audi's in the list:`);
for (let ab = 0; ab < BMWAndAudi.length; ab++) {
    console.log(BMWAndAudi[ab]);
}
